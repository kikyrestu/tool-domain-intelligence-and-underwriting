...
from app.routes.crawl import _background_whois, _background_wayback, _background_score
...
@router.post("/candidates/bulk-rdap")
async def bulk_rdap_candidates(
    request: Request,
    ...
    if ids:
        int_ids = [int(i) for i in ids]
        background_tasks.add_task(_background_whois, None, int_ids)
    return RedirectResponse(url="/candidates", status_code=303)


@router.post("/candidates/bulk-wayback")
async def bulk_wayback_candidates(
    request: Request,
    background_tasks: BackgroundTasks,
):
    form = await request.form()
    ...
    if ids:
        int_ids = [int(i) for i in ids]
        background_tasks.add_task(_background_wayback, None, int_ids)
    return RedirectResponse(url="/candidates", status_code=303)


@router.post("/candidates/bulk-score")
async def bulk_score_candidates(
    ...
    if ids:
        int_ids = [int(i) for i in ids]
        background_tasks.add_task(_background_score, None, int_ids)
    return RedirectResponse(url="/candidates", status_code=303)
